  ...
  if [ $GITHUB_RELEASE_ENABLED = 1 ]; then
    ...
    if [ $get_release_tag == "404" ]; then
      ...
      release_json="{ \
        ...
        \"name\": \"$app_ver\", \
        \"body\": \"Download from [Bintray](https://bintray.com/obsidiandynamics/kafdrop/download_file?file_path=com%2Fhomeadvisor%2Fkafka%2Fkafdrop%2F${app_ver}%2Fkafdrop-${app_ver}.jar)\", \
        \"draft\": false, \
        \"prerelease\": false \
      }"
      set +x
